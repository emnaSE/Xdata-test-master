...
# xdata_hash
RUN git clone https://github.com/dashpay/xdata_hash
RUN cd xdata_hash && python3 setup.py install
...
RUN groupadd -g ${GROUP_ID} xdata
RUN useradd -u ${USER_ID} -g xdata -s /bin/bash -m -d /xdata xdata
...
RUN mkdir /xdata-src && \
  ...
  chown $USER_ID:$GROUP_ID /xdata-src && \
