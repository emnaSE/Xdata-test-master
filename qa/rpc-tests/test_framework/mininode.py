...
def xdatahash(s):
    return xdata_hash.getPoWHash(s)
...
# Objects that map to xdatad objects, which can be serialized/deserialized
...
class CBlockHeader(object):
    ...
    def calc_sha256(self):
        if self.sha256 is None:
            ...
            r += struct.pack("<I", self.nBits)
            r += struct.pack("<I", self.nNonce)
            self.sha256 = uint256_from_str(xdatahash(r))
            self.hash = encode(xdatahash(r)[::-1], 'hex_codec').decode('ascii')
...
class msg_headers(object):
    ...
    def deserialize(self, f):
        # comment in xdatad indicates these should be deserialized as blocks
