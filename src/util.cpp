...
const char * const BITCOIN_CONF_FILENAME = "xdata.conf";
const char * const BITCOIN_PID_FILENAME = "xdatad.pid";
    ...
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL || *cacheCounter != logAcceptCategoryCacheCounter.load())
        {
            ...
            if (mapMultiArgs.count("-debug")) {
                ...
                // "xdata" is a composite category enabling all Xdata-related debug output
                if(ptrCategory->count(std::string("xdata"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\XdataCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\XdataCore
    // Mac: ~/Library/Application Support/XdataCore
    // Unix: ~/.xdatacore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "XdataCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/XdataCore";
#else
    // Unix
    return pathRet / ".xdatacore";
#endif
#endif
}
...
void ReadConfigFile(const std::string& confPath)
{
    ...
    if (!streamConfig.good()){
        // Create empty xdata.conf if it does not excist
...

    {
        ...
        {
            // Don't overwrite existing settings so command line settings override xdata.conf
