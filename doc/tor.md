...
port 9050 and only allows .onion nodes to connect:
	...
	./xdatad -onion=127.0.0.1:9050 -onlynet=tor -listen=0 -addnode=ssapp53tmftyjmjb.onion

In a typical situation, this suffices to run behind a Tor proxy:

	./xdatad -proxy=127.0.0.1:9050
...
config file):
	...
	HiddenServiceDir /var/lib/tor/xdatacore-service/
	HiddenServicePort 8117 127.0.0.1:8117
	HiddenServicePort 18177 127.0.0.1:18177

The directory can be different of course, but (both) port numbers should be equal to
your xdatad's P2P listen port (8117 by default).

	-externalip=X   You can tell Dash Core about its publicly reachable address using
	                this option, and this can be a .onion address. Given the above
	                configuration, you can find your onion address in
	                /var/lib/tor/xdatacore-service/hostname. Onion addresses are given
...
In a typical situation, where you're only reachable via Tor, this should suffice:
	...
	./xdatad -proxy=127.0.0.1:9050 -externalip=ssapp53tmftyjmjb.onion -listen

(obviously, replace the Onion address with your own). It should be noted that you still
listen on all devices and another node could establish a clearnet connection, when knowing
your address. To mitigate this, additionally bind the address of your Tor proxy:

	./xdatad ... -bind=127.0.0.1
...
and open port 8117 on your firewall (or use -upnp).

If you only want to use Tor to reach onion addresses, but not use it as a proxy
for normal IPv4/IPv6 communication, use:

	./xdatad -onion=127.0.0.1:9050 -externalip=ssapp53tmftyjmjb.onion -discover
...
configured. For cookie authentication the user running xdatad must have write access 
to the `CookieAuthFile` specified in Tor configuration. In some cases this is 
preconfigured and the creation of a hidden service is automatic. If permission problems 
are seen with `-debug=tor` they can be resolved by adding both the user running tor and 
the user running xdatad to the same group and setting permissions appropriately. On 
Debian-based systems the user running xdatad can be added to the debian-tor group, 
