...
- [Building Dash Core](#building-xdata-core)
...
git clone https://github.com/dashpay/xdata
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/xdata/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/dashpay/xdata
...
./bin/gbuild --commit xdata=${COMMIT} --url xdata=${URL} ../xdata/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit xdata=${COMMIT} --url xdata=${URL} ../xdata/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit xdata=${COMMIT} --url xdata=${URL} ../xdata/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the xdata git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=bionic on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../xdata/contrib/gitian-descriptors/*|sort|uniq )
...
BTCPATH=/some/root/path/xdata
SIGPATH=/some/root/path/xdata-detached-sigs

./bin/gbuild --url xdata=${BTCPATH},signature=${SIGPATH} ../xdata/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/xdata-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/xdata-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/xdata-osx-build.assert
```
...
[xdata/gitian.sigs](https://github.com/dashpay/gitian.sigs/) repository, or if that's not possible to create a pull
request.
