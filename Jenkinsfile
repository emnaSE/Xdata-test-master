...
for(int i = 0; i < targets.size(); i++) {
  ...
  tasks["${target}"] = {
    node {
      ...
      withEnv(env) {
        def builderImageName="xdata-builder-${target}"
        ...
        builderImage.inside("-t") {
          ...
          sh "cp -ra ${pwd}/. /xdata-src/"

          // restore cache
          def hasCache = false
          try {
            copyArtifacts(projectName: "dashpay-xdata/${BRANCH_NAME}", optional: true, selector: lastSuccessful(), filter: "ci-cache-${target}.tar.gz")
          } catch (Exception e) {
          }
          if (fileExists("ci-cache-${target}.tar.gz")) {
            hasCache = true
            echo "Using cache from dashpay-xdata/${BRANCH_NAME}"
          } else {
            try {
              copyArtifacts(projectName: 'dashpay-xdata/develop', optional: true, selector: lastSuccessful(), filter: "ci-cache-${target}.tar.gz");
            } catch (Exception e) {
            }
            if (fileExists("ci-cache-${target}.tar.gz")) {
              hasCache = true
              echo "Using cache from dashpay-xdata/develop"
            }
          }

          if (hasCache) {
            sh "cd /xdata-src && tar xzf ${pwd}/ci-cache-${target}.tar.gz"
          } else {
            sh "mkdir -p /xdata-src/ci-cache-${target}"
          }

          stage("${target}/depends") {
            sh 'cd /xdata-src && ./ci/build_depends.sh'
          }
          stage("${target}/build") {
            sh 'cd /xdata-src && ./ci/build_src.sh'
          }
          stage("${target}/test") {
            sh 'cd /xdata-src && ./ci/test_unittests.sh'
          }
          stage("${target}/test") {
            sh 'cd /xdata-src && ./ci/test_integrationtests.sh'
          }

          // archive cache and copy it into the jenkins workspace
          sh "cd /xdata-src && tar czfv ci-cache-${target}.tar.gz ci-cache-${target} && cp ci-cache-${target}.tar.gz ${pwd}/"
        }
